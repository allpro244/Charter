...
Cycle model: regime-switching (expansion, late cycle, recession, recovery) with seeded shocks. The economy draws from its own seeded stream (`rngEconomy`), so bank behavior never changes the macro path. Some recessions are banking crises (credit-driven, many failures, S&L or 2008 shape); most are not. Sector indices have their own paths and correlations (energy follows oil, tech is high beta, government is flat). Generator runs forever.
...
Decision path: above the dial, pause with the memo: approve / counter (rate, LTV, term, guarantee) / decline. Below the dial, auto-decide under written policy with error driven by CCO skill. The dial follows capital by default (5% of tier 1) and the player may hand credits up to three times the line to a loan committee that approves the sound ones within policy (D52).
...
Owns: deposits by type per branch, rate sheet, betas, stickiness, runs, brokered, FHLB capacity, fed funds, later swaps. Branch purchases (D59): the deposits move with cash less a premium at the `branchDepositPremium` band; the premium is goodwill. Large depositors (D62): negotiated accounts at the `largeDepositPremium` band over the market for a term, carried in the targets while confidence holds. Operating accounts (D58): the checking target carries the `operatingBalanceShare` band of the business loan book. Branch inflows go to the branches with room under their own targets.
