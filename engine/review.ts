...
export function reviewQuarterly(ctx: Ctx): void {
  ...
  for (const id of world.bankOrder) {
    ...
    if (b.id === world.playerBankId) {
      ...
      emit(ctx, 'system', `${review.quarter} closed: net income ${money(review.netIncome)}, ROA ${pct(roa)}, provision ${money(review.provision)}, charge-offs ${money(b.is.quarter.chargeOffs)}. Press q for the review.`, {
