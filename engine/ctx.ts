...
export function addPending(ctx: Ctx, p: Omit<Pending, 'id' | 'day' | 'expires' | 'blocking'> & { expires?: number | null; blocking?: boolean }): Pending {
  const item: Pending = { ...p, expires: p.expires ?? null, blocking: p.blocking ?? true, id: nextId(ctx.world, 'p'), day: ctx.world.day };
