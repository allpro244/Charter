...
export function adviceFor(world: World): Card[] {
  ...
  if (lev < 0.06) out.push({ key: 'lev', text: `Leverage ${pct(lev, 1)}: below 5% dividends stop and the examiner arrives. Raise capital on ME or slow lending on LOANS.` });
  ...
  if (a.cash / Math.max(1, assets) < 0.03) out.push({ key: 'cash', text: `Cash is ${pct(a.cash / Math.max(1, assets), 1)} of assets. Below 3% a bad week means borrowing or selling bonds at a loss. See FUND.` });
  const gap = marketDepositRate(world) - bankDepositRate(b);
  if (gap > 0.0075) out.push({ key: 'rates', text: `You pay ${pct(bankDepositRate(b))} against a market at ${pct(marketDepositRate(world))}. Money market and CD money leaves first. FUND has the sheet.` });
  ...
  if (vacancies.length > 0) out.push({ key: 'off', text: `${vacancies.length} officer ${vacancies.length === 1 ? 'seat is' : 'seats are'} vacant (${vacancies.join(', ').toUpperCase()}). A vacant desk runs at a low default skill. OFF has candidates.` });
  if (b.dial.maxAuto === 0 && b.applications.received > 50) out.push({ key: 'dial', text: `The dial is at zero: every loan crosses your desk. Fine at ${short(assets)}; it will not scale. LOANS, policy and dial.` });
  if (b.enforcement !== 'none') out.push({ key: 'enf', text: `Under a ${b.enforcement === 'mou' ? 'memorandum' : b.enforcement === 'consent' ? 'consent order' : 'PCA directive'}: fix the findings on BS before the next exam. Ignored, it escalates.` });
  if (b.forSale === false && b.reviews.length >= 4) {
    const last = b.reviews[b.reviews.length - 1];
    if (last && last.netIncome < 0) out.push({ key: 'loss', text: `Last quarter lost ${short(-last.netIncome)}. QTR attributes every dollar; the losses table names the calls that made them.` });
  }
