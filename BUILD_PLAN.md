...
2026-09-17 | Owner playing the desk: five asks in one session (Overview too wide; money as $50.55MM; growth on Earnings; the Bonds tab; a demand and economy view; a loan rate sheet) | the Overview side column no longer pushes the page wider than the window and table words wrap everywhere so no tab scrolls sideways at 1024 wide and up; every dollar figure carries its scale ($12.3K, $50.55MM, $1.20B); Earnings gained Quarter over quarter and Year over year with change and change % on every statement and balance sheet line, a twelve months against twelve comparison and a quarter by quarter history (the engine now keeps the player's closed-quarter statements, tested against the call reports); Bonds is a rate sheet (six terms by three products, today's yield in each cell, what a one point rise does) plus a pick panel that shows yield, dollars a year, pickup over cash, the price hit, the fee and the exact purchase on the button, and holdings with yield, earnings a year and a total row; a new Economy tab (D46): where to lend ranks every type by borrowers a year, market rate, the loss environment (the visible part of stress, split out of the engine without changing a number), expected loss and margin after losses beside your book and everyone else's, the cycle with its stage, meaning, what follows and what a bank does now, rates, curve, jobs, prices, housing, oil, sectors with the home county's shares; a loan rate sheet on Lending (D47): every type priced against the market in 1bp and 25bp steps, memos carry the offset, arrivals scale by a hand band (loanRateElasticity), a pure test plus a fixture-gated year-long test; stale tab names in the advisor fixed | open: the map and the real world still wait on the data; this session's probes for public mirrors of the county industry mix, CBSA list, FDIC bank data and county home prices are in the log below
