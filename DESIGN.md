...
| D22 | People layer is key officers only: CFO, CCO, CLO, later COO and business line heads | Skill, cost, tenure, loyalty. Can be hired and fired; they ask for raises and resign when disloyal, but no rival poaches them (the owner found poaching a chore, not a threat). |
...
| D35 | Rivals are active antagonists. They price to take deposits, outbid on acquisitions, and open branches against the player (they no longer poach officers, per D22). A bigger, better-run rival can put the player's bank into decline | Replaces the board as the late-game threat. |
...
| D49 | Guard rails and a score for the casual player, all real: the legal lending limit (15% of capital to one borrower) is enforced at the desk and in policy; a desk approval is funded from cash above the cushion and up to half the Home Loan Bank line, drawn on the spot, and a loan beyond both is not made (the button says why); the other half of the line is the contingency funding plan and covers withdrawals; the advisor warns when the line is half used or deposits fell a quarter and are still falling; another bank's forced bond sales and line draws stay out of the feed; a day's batch offers "approve the sound ones" (health 65 and up, within policy); every bank keeps the player's own record (approved, declined, paid off, went bad, lost); the ladder ranks the bank among every bank in America by assets, the other banks grown with the economy, with milestones at the top 1,000, 500, 250, 100, 50, 25, 10, 5 and the top; the Overview's recent feed shows only what touches this bank, the full feed everything; the map says what a branch in any county would gather and cost | A new bank that approved everything it saw died inside a year without knowing why. Real banks have these limits; the desk now shows them. |
| D50 | The bank runs itself between the CEO's decisions, the way a real one does, under three standing policies the CEO sets: the deposit sheet follows the market at per type offsets (the CFO reprices at each month end; the CEO can switch to setting rates by hand), the loan officers book pooled loans under the written policy toward a loans to deposits target (the branch network's lending, in the home county's mix, at most four percent of deposits a month, quality set by the credit officer's skill and the policy's worst grade; the desk still sees every credit above the size line), and the CFO's investment policy buys a chosen product and duration with cash above a cash target. Defaults: market plus zero, 75%, and 8% cash into three year agencies. | A bank whose sheet never moved held 70% of its assets in cash for fifteen years and paid 1.5% on deposits in a zero rate world. Real CEOs set policies and ALCO executes them; the desk is for the exceptions. |
| D51 | What happens when a loan fails is on the desk and the player has the real choices: a loan goes 30, 60 and 90 days late, then nonaccrual (its interest reversed and stopped), workout at six months, and at nine months real estate is foreclosed into REO at liquidation value after local prices and the rest charged off at the loan's own loss given default. Before that the player may sell the note to a distressed debt buyer (expected recovery less 10, 15 or 20 percent by stage: cash now, the shortfall charged off today, the loan gone), sell a foreclosed property now at 90 percent of its carrying value instead of waiting for a buyer, or sell a type's nonperforming pooled balances in bulk (the loss given default's recovery less a fifth). Loans the bank cannot make, over the legal limit or beyond what it can fund today, are turned away at the door and counted, never brought to the desk. | Real banks sell notes and REO and run bulk NPL sales every cycle; a desk that only waits for the charge-off is not a bank. And a decision that cannot be taken is not a decision. |
| D52 | The delegation ladder: the size line follows capital by default (five percent of tier 1 in fifty thousand dollar steps, never under a quarter million, reset each month) until the player sets it by hand; and the player chooses what reaches the desk: every loan, loans above the line (the loan officer decides the rest under policy), or only the biggest (the loan committee decides credits up to three times the line, approving the sound ones within policy and declining the rest, with one summary line a month). Each December the player's bank gets a year in review line and milestone: profit and return on assets, assets and their growth, rank and where it was a year ago, loans approved at the desk and how many went bad. The advisor names the growth moves a strong bank can make (a branch, a bank for sale nearby it can pay for, a first dividend after the de novo years, a CEO salary the board would pay) while it can make them; exam and enforcement cards lead with plain words and point at the tab that fixes each finding; an approval outside policy says so on the button. | A flat quarter million line set for a new bank never grew, so a $500M bank still saw $300K loans a dozen times a month. Real CEOs set lending authority and sit on committee for the large credits; the rest is the officers' job. |
| D53 | The bank remembers its borrowers and its towns. Every borrower the bank lends to is kept by name and county with the count of loans, payoffs and loans gone bad; about a third of the applicants in a county with a record are borrowers the bank knows, back after at least a season, and their record with the bank sets their payment history on the memo (paid us: clean; went bad on us: poor, and a red flag), lifts or cuts the character score on the health meter, and is said in one sentence on the decision card. A county with a player branch keeps thirteen months of its condition; when it moves three percent against the country over a year, one market line names the sector behind it, its share of local jobs, and how many of the bank's borrowers are in it, and then waits six months. Texture: a milestone for every branch opened and for the first dividend, the quarter closed in one plain line (profit, deposits, charge-offs, rank), and the map shaded by the player's share of each county's deposits. | Community banking is relationships; a stranger every time is not it. The county data is the game's best asset and the loan book never spoke of it. Small moments are what a casual player remembers. |
| D54 | One book at two levels of detail, never two books. The Loans screen shows every loan that came through the application desk as its own file (decided by the player, by the loan officer under policy, or inherited with the bank), and directly below it the officers' lending under policy as the rest of the book: one line per loan type with loans, balance, rate, weak share, not paying share and losses, opening to one line per year made. The two sections sum to the balance at the top. The bulk sale of a type's nonperforming pooled loans sits on that type's line. No representative loans are generated for a pool | A CEO reads the files that crossed the desk and reads the rest of the book as a portfolio report; that is neither all loans nor none, it is how a bank is actually seen from the top. Invented sample loans looked like loans the player could not see, which was the confusion. Rule 4 fixes the two representations in the engine; the desk should not make the seam a feature. |
| D55 | Where to open a branch is a ranked choice, not a hunt across three thousand counties. The Map tab lists the ten counties where a new branch would earn the most in a mature year (the bank's own margin on the deposits the branch could hold, less its running cost), with distance from home, the county's deposit pool, other banks' branches there, what one branch could gather when mature alone and its share against the banks already there (the county contest's own split, D36), the year it pays for itself, and an Open button that names the premises cost. A county's card shows the same case from the same formula the branch will run on (D36's target, uncontested), and the Open button is disabled with the reason when the bank cannot open there (a branch already, no data, the enforcement order, not enough cash). Big-city dots no longer catch the click meant for their county, and no key opens a branch. A branch's reach falls off with distance from home on a scale that grows with the bank's size (`branchReachKm`, a hand band: 400 km for a $100M bank, scaling with the fourth root of assets), so a small bank's best openings are nearby and a national bank's are anywhere. | The old card promised deposits the branch could not reach and the button did nothing when it could not open. A CEO picks from a shortlist the CFO ranked. |
...
## Rules
- Mouse first, keyboard second. Every screen is a tab in a bar at the top; every action is a button; every decision is a dialog with buttons. Keyboard shortcuts remain for the same things; their hints show only while Help is open.
- A persistent top bar carries the bank's name, the date, the clock (pause, play, three speed buttons; keys 1 to 5 reach every speed) and the numbers a CEO watches every day: assets, capital, cash, rank, net worth, with a status pill for the capital category.
...
- A decision that pauses the clock docks under the tabs on every screen with large labelled buttons. A loan decision opens compact: the health score and word, one sentence of verdict (purpose, term, rate, the weak spots, the strengths), whether the rate pays, the CCO's grade, and the buttons; the banker's full read (the five Cs table) opens on request. The card carries a button that doubles the size line for a player seeing too many loans. A decision that does not pause the clock (an offer) sits beside the feed as a card, and the Home tab shows a count.
...
- The map is the one graphical surface: county outlines, metro dots by real population, branches, rivals, shading by your deposit share, a sector or condition chosen with buttons, and under it the ranked list of where to open next (D55). Hover shows the county's real stats; a button opens a branch.
...
- At most two confirmations show at once, bottom right, and each is gone in a moment; the feed is the record.
- No em dashes or en dashes.

## Layers
The desk answers the player's questions before it shows the schedules. Overview answers "how am I doing" with five gauges (capital, cash, loans, profit, growth), each with a plain word, a number, a status pill, one sentence of meaning and a link to the tab that changes it; a profit summary in plain words; sparklines; what to do next; the recent feed. Every screen opens with a summary and offers its detail on request (every line, the regulators' view, the pools). Every bank word carries a plain explanation on hover, from a glossary in `ui/glossary.ts`. Every decision shows "what this means" first, computed only from what the player can see. Every loan decision carries a health meter: the five Cs (capacity, collateral, character, capital, conditions) and concentration scored 0 to 100 from the memo with a plain note each, an overall word, the CCO's grade beside it, and a separate line on whether the rate pays for the risk; a batch shows the meter for every loan in it. Every adjustable number moves by a fine step and a coarse step, and every amount can be typed. Every action answers with a short confirmation.

## Screens (v1)
Six tabs, grouped by the question a player is asking. Home (today: what to do next, the gauges, profit in plain words, your calls, the recent feed; results: this period, where it came from, quarter over quarter, year over year), Loans (your book: every loan that crossed the desk as a file and the rest of the book by type and year on one screen; rate sheet; policy and dial with the delegation ladder), Money (deposits and cash, bonds, balance sheet and capital), World (the economy now, then the economy: where to lend, the cycle, sectors and your market; the other banks: the ones in your market, for sale nearby and the largest in your state, with every bank on request, plus business lines and abroad), Map, You (your money and the ladder; your team). Debug sits behind Help. The old screen names still work as links.
